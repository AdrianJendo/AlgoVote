...
const EnterVoteInfo = ({ handleNext, handleBack }) => {
  ...
  if (voteResults.voteStatus === "register") {
    ...
    generalInfo.push({
      ...
      data: `${
        Math.round((voteResults.numVoted / voteResults.numRegistered) * 10000) /
        100
      }%`,
    });
    Object.keys(voteResults.candidates)
  ...
  generalInfo.push({
    ...
    data: `${
      Math.round((voteResults.numRegistered / voteResults.numVoters) * 10000) /
      100
    }%`,
  });
    ...
    >
      <Typography variant="h4" sx={{ flexGrow: 1, padding: "20px" }}>
        ...
        <IconButton
          onClick={async () => {
            setVoteRefresh(true);
            const lookupSuccess = await lookupVote(voteResults, setVoteResults);
            if (lookupSuccess) {
